...
**Lo que se mantiene** vs. el plugin completo en Code: detección de entregables, ruteo por typeahead con fallback a Inbox Claude, creación de tarea con campos clave, custom fields de tiempo.
...
1. RUTEO
   ...
   - Si no hay match o es ambiguo → fallback al proyecto "Inbox Claude" (gid 1214869577300320).

2. CREAR TAREA con asana_create_task:
   - name: "<verbo + objeto> · <duración>". Duración: "45min" si <1h, sino "1h20".
   - notes (markdown):
       **Resumen:** <2-3 líneas describiendo el trabajo>
...
3. CONFIRMAR
   - Si la creación es exitosa: respondé brevemente con el permalink_url devuelto.
...
- **Tokens**: este bloque ocupa ~600 tokens. En Global Instructions se carga en cada sesión de Cowork. Si te resulta mucho, movelo a Project Instructions de proyectos específicos.
